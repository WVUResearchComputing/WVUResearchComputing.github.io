...
AMBER was compiled using GCC 9.3 and MPICH 3.4.1.
The first build is done without CUDA and with very small adjustments the CUDA version can be build from a GPU compute node.
...
Using SSH, go to a GPU compute node, where the CUDA Toolkit is installed, and load the module for CUDA 11.3::

  module load parallel/cuda/11.3

Delete the file ``CMakeCache.txt`` before running the script again.
That will recreate the file with values for the CUDA compilation.
