...
.. list-table:: User Commands
   ...
   * - ``_gpu``
     - Compute nodes associated to the partition one or more GPU cards
   * - ``_day``
     - The partition has a walltime of maximum 1 day
   * - ``_week``
     - The partition has a walltime of maximum 1 week
   * - ``comm_gpu_inter``
     - Special partition offering compute nodes with GPU for interactive jobs, walltime is 4 hours
...
They also could contain the name of the job, specification for sending emails when the job start, ends of fails, where the output of the script wil go and other configurations that will be used before, during and after the job enters in execution.
...
For example, if the job ID were 120049. The output produce by the submission script will contain::

	$> cat slurm-120049.out 
...
The next 4 lines all start with ``$SBATCH``::
...
The lines will each in order, set the name of the job, the number of compute nodes, the number of CPU cores on the node and the partition selected for the job.
Except for the job name which is undefined by default, all other values here correspont to the default values.
Those 3 lines for the number of nodes, cores and partition can be removed from the submission script and the script will take assume the default values.
...
In the final 5 lines we are demonstrating the used of some Environment variables that are created when the job start running on the compute node.
In this case we are revelaling the content of those variables and writing those along with the output of the script.
The output is all that the script or the programs call by the script writes on the standard output.
If the script were executed directly, the standard output is the terminal window. 
In the case of a batch scrip, all the output is directed to a couple of files.
...
We will explore more complex submission scripts but first lets explore how to monitor the jobs submitted and how to cancel jobs.
