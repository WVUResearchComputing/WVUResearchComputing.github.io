...
The submission script could be written like this::
    ...
    echo "Number of CPUs per task: $SLURM_CPUS_ON_NODE"
...
The next 5 lines all start with ``$SBATCH``::
...
The lines will each in order, set the name of the job, the number of compute nodes, the number of tasks, the number of CPUs per task and the partition selected for the job.
Except for the job name which is undefined by default, all other values here correspont to the default values.
Those 4 lines for the number of nodes, tasks, CPU cores and partition can be removed from the submission script and the script will take assume the default values.
...
Lets consider a variation of the submission script were by mistake we will be asking for several compute nodes::
