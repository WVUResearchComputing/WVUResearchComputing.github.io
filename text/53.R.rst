...
There are two main ways to install R packages from CRAN. You can install packages from inside the interactive R interpreter. Execute the command `R` and from there install the package with::
    ...
    install.packages('<Package Name>', repos="file:///gpfs21/mirror.math.princeton.edu/pub/CRAN")

Another alternative is using the command `Rscript`, this time from the normal shell::

    $> Rscript -e 'install.packages("<PACKAGE>", repos="file:///gpfs21/mirror.math.princeton.edu/pub/CRAN")'

With those commands, you will automatically download the package from CRAN and eventually all dependencies needed for that package. The declaration ``repos="file:///gpfs21/mirror.math.princeton.edu/pub/CRAN"`` is optional. 
