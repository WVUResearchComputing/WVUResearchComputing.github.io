...
Next, we discuss :ref:`ad-python`, the different environment modules for Python and how to install packages as user with pip. We will also cover the use of IPython for interactive computing.

On section :ref:`ad-r`, the R versions available and how to install packages as user from the Comprehensive R Archive Network (CRAN), a repository of R packages. We also cover RStudio, a user friendly interface for the R language.

The Comprehensive Perl Archive Network (CPAN) is a repository of software modules and accompanying documentation of codes written in the Perl programming language. :ref:`ad-perl` shows how to install perl packages from CPAN as user.
...
Finally, section :ref:`ad-matlab` shows how to run Matlab non-interactively with a couple of examples.
