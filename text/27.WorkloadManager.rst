...
Notice that `srun` is actually doing a double function. From one side is creating a new job (In the case above the job with ID=22432) followed by a terminal session on the machine assigned to the job.
In the example above the job is requesting default values.
...
You launch an **interative job** when you expect to execute the work step by step, controlling the execution of tasks and receiving the output as soon it is produced.
...
When we know all the steps needed to achive a result we can submit **non-interactive jobs**.
**Non-iteractive jobs** execute a complete set of tasks without human intervention.
...
An HPC cluster can manage many jobs being sumitted and that is how you get the best outcome from that machine.
